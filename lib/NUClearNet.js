...
NUClearNet.prototype.send = function (typeName, data, target, reliable) {
    ...
    if (reliable) {
        ...
        for (var i = 0; i < this.targets.length; i++) {
            ...
            if (target === undefined || this.targets[i].name === target) {
                // TCP should only have one packet
                console.log("Sending unicast tcp message to", this.targets[i].name, packet.unpack(messages[0]));
                this.targets[i].tcpSocket.write(messages[0]);
            }
        }
    }
    else {
        ...
        if (target === undefined) {
            ...
            for (var i = 0; i < messages.length; i++) {
                console.log("Sending multicast message", packet.unpack(messages[i]));
                this.udpSocket.send(messages[i], 0, messages[i].length, this.multicastPort, this.multicastGroup);
            }
        }
        else {
            ...
            for (var i = 0; i < this.targets.length; i++) {
                // Send if we are sending to all, or to this node
                if (this.targets[i].name === target) {
                    for (var j = 0; j < messages.length; j++) {
                        console.log("Sending unicast udp message to", this.targets[i].name, packet.unpack(messages[j]));
                        this.udpSocket.send(messages[j], 0, messages[j].length, this.targets[i].udpPort, this.targets[i].address);
                    }
                }
            }
        }
