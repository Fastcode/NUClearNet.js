    ...
    if (packetSize <= target.tcpBuffer.length) {
        ...
        if (obj.type === packet.ANNOUNCE) {
            ...
            if (this._udpTargets[udpKey] === undefined) {
                ...
                this._emit('nuclear_join', target.name, target.address, target.tcpPort, target.udpPort);
            }
            else {
                // We already have one
...
NUClearNet.prototype._tcpClose = function(addr) {
    ...
    this._emit('nuclear_leave', target.name, target.address, target.tcpPort, target.udpPort);
};
