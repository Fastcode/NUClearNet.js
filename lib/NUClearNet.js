...
NUClearNet.prototype.tcpClose = function(addr) {
    ...
    var nameIndex = this.nameTargets[target.name].indexOf(target);
    if (nameIndex >= 0) this.nameTargets[target.name].splice(nameIndex, 1);
    ...
    if (reliable) {
        ...
        if (target === undefined) {
            ...
            messages.forEach(function (m) {
                this.udpSocket.send(m, 0, m.length, this.multicastPort, this.multicastGroup);
            }, this);
        }
        else {
            ...
            this.targets.forEach(function (t) {
                ...
                if (t.name === target) {
                    ...
                    messages.forEach(function (m) {
                        this.udpSocket.send(m, 0, m.length, t.udpPort, t.address);
                    }, this);
                }
            }, this);
        }
