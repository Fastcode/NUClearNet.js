...
var NUClearNet = function(name, multicastGroup, multicastPort) {
    ...
    this._multicastSocket.on('listening', function () {
        ...
        Object.keys(networkInterfaces).forEach(function (key) {
            networkInterfaces[key].forEach(function (iface) {
                if (iface.family === 'IPv4' && !iface.internal) {
                    // Add membership to all interfaces
                    this._multicastSocket.addMembership(this._multicastGroup, iface.address);
                }
            }, this);
        }, this);
    }.bind(this));
    ...
    this.on('newListener', function (event) {
        ...
        // If we are listening to a new event
        if(event !== 'nuclear_join'
         && event !== 'nuclear_leave'
         && event !== 'nuclear_listen'
         && event !== 'nuclear_unlisten'
         && event !== 'newListener'
         ...
         && this.listenerCount(event) === 0) {
            ...
            // Emit an event that we are not listening to this
            this.emit('nuclear_listen', event);
        }
    }.bind(this));
    ...
    this.on('removeListener', function (event) {
        ...
        if(event !== 'nuclear_join'
         ...
         && this.listenerCount(event) === 0) {
            ...
            // Emit an event that we are listening to this
            this.emit('nuclear_unlisten', event);
        }
    }.bind(this));
};
    ...
    if (packetSize <= target.tcpBuffer.length) {
        ...
        if (obj.type === packet.type.ANNOUNCE) {
            ...
            if (this._udpTargets[udpKey] === undefined) {
                ...
                // We already made a connection to this remote
...
NUClearNet.prototype._udpHandler = function (msg, remote) {
    ...
    if (obj.type === packet.type.ANNOUNCE) {
        ...
        if(!(obj.name === this._name && obj.tcpPort === this._tcpPort && obj.udpPort === this._udpPort)) {
            // Check if this packet is suspect
            if(remote.port !== obj.udpPort) {
                return;
            }

            // Work out our UDP key
            var udpKey = remote.address + ":" + obj.udpPort;

            // Look for this in our existing connections
            if (this._udpTargets[udpKey] === undefined) {

                // We are not connected to this, make a connection and register it
                var socket = net.connect(obj.tcpPort, remote.address, function () {
                    // Send an announce packet to the remote when we are connected
                    var p = packet.packAnnounce(this._name, this._tcpPort, this._udpPort);
                    socket.write(p);

                    // Clear out our default error handler
                    socket.removeAllListeners('error');

                    // Register our connection
                    var target = this._tcpConnection(socket);

                    // This is a new connection record the details
                    target.udpPort = obj.udpPort;
                    target.tcpPort = obj.tcpPort;
                    target.name    = obj.name;
                    target.address = remote.address;
                ...
                // Add an error handler so that if this socket fails to connect we survive
                socket.on('error', function() {
                    console.log('Failed to connect to', remote.address, obj.tcpPort);
                });
            }
        }
    }
...
NUClearNet.prototype._tcpConnection = function (socket) {
    ...
    socket.on('error', this._tcpClose.bind(this, addr));

    return this._tcpTargets[key];
};
