    ...
    if (packetSize <= target.tcpBuffer.length) {
        ...
        if (obj.type === 'ANNOUNCE') {
            ...
            if (this.udpTargets[udpKey] === undefined) {
                ...
                // Run our connection callbacks
                for (var i = 0; i < this.joinCallbacks.length; i++) {
                    this.joinCallbacks[i]({
                        name: target.name,
            ...
            var callbacks = this.callbackMap[obj.hash];
...
NUClearNet.prototype.tcpClose = function(addr) {
    ...
    for (var i = 0; i < this.leaveCallbacks.length; i++) {
        this.leaveCallbacks[i]({
            name: target.name,
            address: target.address,
            tcpPort: target.tcpPort,
            udpPort: target.udpPort
...
NUClearNet.prototype.onJoin = function (callback) {
    this.joinCallbacks.push(callback);
};

NUClearNet.prototype.onLeave = function (callback) {
    this.leaveCallbacks.push(callback);
};

NUClearNet.prototype.on = function (name, callback) {

    // Murmurhash our name
    var hash = murmurhash3(name, 'ascii', 0x4e55436c).toString('hex');

    // Lazy initialise
    if (this.callbackMap[hash] === undefined) {
        this.callbackMap[hash] = [];
    }

    // Add the callback
    this.callbackMap[hash].push(callback);
};
